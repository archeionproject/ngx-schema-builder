...
describe('getTypeValidation', () => {
  ...
  it('falls back to z.unknown for unknown types', () => {
    expect(getTypeValidation('boolean', t).safeParse('anything').success).toBe(
      true,
    );
  });
});
...
describe('validateSchemaByType', () => {
  it('passes a consistent string schema', () => {
    expect(
      validateSchemaByType({ minLength: 1, maxLength: 5 }, 'string', t).success,
    ).toBe(true);
  });
  ...
  it('rejects a negative / non-integer length', () => {
    expect(validateSchemaByType({ minLength: -1 }, 'string', t).success).toBe(
      false,
    );
    expect(validateSchemaByType({ minLength: 1.5 }, 'string', t).success).toBe(
      false,
    );
  });
  ...
  it('rejects number with both inclusive and exclusive minimum', () => {
    expect(
      validateSchemaByType({ minimum: 1, exclusiveMinimum: 0 }, 'number', t)
        .success,
    ).toBe(false);
  });
  ...
  it('rejects number with min > max', () => {
    expect(
      validateSchemaByType({ minimum: 10, maximum: 1 }, 'number', t).success,
    ).toBe(false);
  });

  it('rejects enum values outside the numeric range', () => {
    expect(
      validateSchemaByType({ minimum: 0, maximum: 5, enum: [10] }, 'number', t)
        .success,
    ).toBe(false);
  });
  ...
  it('rejects multipleOf <= 0', () => {
    expect(validateSchemaByType({ multipleOf: 0 }, 'number', t).success).toBe(
      false,
    );
  });
  ...
  it('validates array min/max items and contains', () => {
    expect(
      validateSchemaByType({ minItems: 2, maxItems: 1 }, 'array', t).success,
    ).toBe(false);
    expect(
      validateSchemaByType({ minContains: 2, maxContains: 1 }, 'array', t)
        .success,
    ).toBe(false);
  });
  ...
  it('validates object min/max properties', () => {
    expect(
      validateSchemaByType({ minProperties: 5, maxProperties: 1 }, 'object', t)
        .success,
    ).toBe(false);
  });
});
...
describe('buildValidationTree', () => {
  ...
  it('derives type from a string[] type and defaults to object', () => {
    expect(
      buildValidationTree({ type: ['object', 'null'] } as never, t).name,
    ).toBe('object');
